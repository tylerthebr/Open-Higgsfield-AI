...
export async function generateVideo(params) {
  const { prompt, apiKey, model = 'higgsfield-1', duration = 5, aspectRatio = '16:9', resolution = '720p' } = params;
 ...
 */
export async function pollVideoJob(jobId, apiKey, onProgress, intervalMs = 3000, maxAttempts = 100) {
  let attempts = 0;
  ...
  return new Promise((resolve, reject) => {
    const poll = async () => {
      ...
      try {
        const statusData = await getVideoStatus(jobId, apiKey);
 ...
 */
export const MODELS = [
  { id: 'higgsfield-1', name: 'Higgsfield 1', description: 'Standard quality, fast generation' },
  { id: 'higgsfield-1-pro', name: 'Higgsfield 1 Pro', description: 'High quality, cinematic output' },
];
